        ...
        if (!event.email || event.email == record.email) {
            ...
            if (event.details) {
                ...
                if (event.message == "Submitted Data") {
                    ...
                    results += '<div class="timeline-replay-button"><button onclick="creds_false_positive(' + i + ')" class="btn btn-success">'
...
function creds_false_positive(rid, cid) {
    Swal.fire({
        title: "Are you sure?",
        text: "This result will be flagged as false_positive (RID: " + rid + ")",
        type: "question",
        animation: false,
        showCancelButton: true,
        confirmButtonText: "Continue",
        ...
        if (result.value){
            api.campaignId.get(cid).success((function(c) {
                report_url = new URL(c.url)
                report_url.pathname = '/false_positive'
                report_url.search = "?rid=" + rid
                $.ajax({
                    url: report_url,
                    method: "GET",
