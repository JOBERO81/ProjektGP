...
type Result struct {
	...
	FalsePositiv bool      `json:"falsepositiv" sql:"not null"`
	BaseRecipient
}
...
// HandleCredsFalsePositive updates a Result in the case where credentials are clearly
// invalid and set Credentials false_positive so they not longer count as submitted
// phishing email using the HTTP handler.
func (r *Result) HandleCredsFalsePositive(details EventDetails) error {
	event, err := r.createEvent(EventFalsePostive, details)
	if err != nil {
		return err
	}
